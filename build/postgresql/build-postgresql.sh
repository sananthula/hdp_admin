...
# Setup users access for PostgreSQL. 
	...
	if [ -f ${ETCDIR}/pg_hba.conf ]; then
		sudo cp ${ETCDIR}/pg_hba.conf /var/lib/pgsql/data
		sudo chown postgres:postgres /var/lib/pgsql/data/pg_hba.conf
	fi

	# This opens the listener to all hosts
	if [ -f ${ETCDIR}/postgresql.conf ]; then
		sudo cp ${ETCDIR}/postgresql.conf /var/lib/pgsql/data
...
# Case statement for add or delete user
        case "${OPTION}" in
                ...
                build)
                        checkArg 2
			checkFile $ETCDIR/postgresql.conf 
			checkFile $ETCDIR/pg_hba.conf
			installPostgreSQL
			initPostgreSQL
			configPostgreSQL
			createPostgreSQL
