...
BENCHLOG="${LOGDIR}/benchmark-${DATETIME}.log"
...
# List and select the sizes for the benchmark
	...
	echo ${SIZE} > ${HOME}/.terasize
}

function setTera() {
# Set the parameters for the benchmark

	SIZE=$(cat ${HOME}/.terasize)
...
# Run Validate
	...
	time yarn jar ${TERAJAR} teravalidate \
		...
		${OUTPUTDIR} ${REPORTDIR} >> ${BENCHLOG} 2>&1
}
...
# This function loops the use of TeraSort
	...
	for i in {16,32,64}; do
			...
			time yarn jar ${TERAJAR} teragen \
				...
				${BENCHDIR}/loop/input-$i-$j 1>${BENCHLOG}-$i-$j
			...
			time yarn jar ${TERAJAR} terasort \
				...
				${BENCHDIR}/loop/input-$i-$j ${BENCHDIR}/loop/output-$i-$j 1>>${BENCHLOG}-$i-$j

			# Run the validate to create a record
			time yarn jar ${TERAJAR} teravalidate \
				-Dmapreduce.map.memory.mb=2048 \
				-Dmapreduce.reduce.memory.mb=2048 \
				${BENCHDIR}/loop/input-$i-$j ${BENCHDIR}/loop/report-$i-$j >> ${BENCHLOG} 2>&1
